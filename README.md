...
| **Agent delete + restore** | `cerefox_delete_document` (soft, requires the caller's read-hash, reason recorded in audit) and `cerefox_restore_document` (its audited inverse) — v1.7.0. Permanent purge stays web-UI-only with human confirmation. |
| **Link integrity** | Every write validates `[Text](uuid)` document links and rejects links to nonexistent ids (mangled-UUID protection — LLMs regenerating text corrupt long random ids at a structural rate); examples escape via code formatting; `[[wikilinks]]` may dangle; `cerefox document dead-links` sweeps for legacy dead links. See [docs/guides/linking.md](docs/guides/linking.md) — v1.7.x |
...
| **Review workflow** (optional) | Store-level switch `review_workflow_enabled` (off on a fresh install). On: agent writes land `pending_review` for a person to approve; filterable on search. Off: no surface shows or enforces a review status (the status is still recorded, so turning it back on shows what was there). Never gates retrieval |
...
same 15 core tools in-process (plus 4 document-relation tools, hidden until you enable them with `cerefox config set relations_enabled true`) — lower latency, no per-call Edge Function billing, and
