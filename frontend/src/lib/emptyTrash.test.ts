...
describe("emptyTrash", () => {
  ...
  test("re-lists past the server cap until nothing eligible comes back", async () => {
    ...
    const confirmed = await t.listTrash();
    // The user confirmed "3 or more"; the cutoff is the newest of those 3, so
    // only rows trashed no later than it are eligible on later passes.
    const result = await emptyTrash({ ...t.deps, confirmed });
    expect(result.purged).toBe(3);
    expect(t.trash.size).toBe(4);
  });
  ...
  test("a fatal purge error aborts the run instead of failing every remaining document", async () => {
    const t = fakeTrash(["a", "b", "c", "d"], { failing: new Set(["b", "c", "d"]) });
    const confirmed = await t.listTrash();
    let calls = 0;
    ...
    expect(result.aborted).toBe("refused b");
    expect(result.failures).toEqual([]);
    expect(calls).toBe(2); // a, then b aborted; c and d never attempted
