...
Cerefox is a persistent, shared knowledge base. You have **18 MCP tools** (17 of them have CLI equivalents — `cerefox_get_help` is MCP-only). For the full guide, search Cerefox for "How AI Agents Use Cerefox" or call `cerefox_get_help` to retrieve this content over MCP.
...
| `cerefox_delete_document` | **Soft**-delete a document (to web-UI trash; excluded from search; only a human can restore or purge) | `document_id`, `expected_content_hash` (**required** — a delete must follow a read), `reason` (recorded in the audit log — give one), `author` |
...
7. **Deletes are soft (recoverable); restore and purge are web-UI-only.** `cerefox_delete_document` requires the document's `content_hash` as you read it (read before you delete) and takes a `reason` — give one; it is what the human reviewing the trash sees. Always surface a delete to the user (`I soft-deleted X — recoverable from the Cerefox web UI trash`). You cannot un-do your own delete from agent code by design.
