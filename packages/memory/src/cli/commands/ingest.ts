...
import { escapedContentNote } from "../../../../../_shared/mcp-tools/escape-heuristic.ts";
...
async function action(
  ...
  try {
    ...
    // #222: non-blocking escaped-content heuristic — same signal the MCP
    // write responses carry. File-based ingests can still ship literal \n
    // when the FILE was generated by an over-escaping process.
    const escapeNote = escapedContentNote(content, "cli");
    if (escapeNote) warn(escapeNote.trim());
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
