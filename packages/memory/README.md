...
| `cerefox mcp` | Local stdio MCP server. Drop-in for Claude Code, Cursor, Claude Desktop, Codex CLI, Gemini CLI. Exposes the same 14 core MCP tools as the remote `cerefox-mcp` Edge Function, plus 4 document-relation tools that stay hidden until enabled. |
...
the 14 core MCP tools (`cerefox_search`, `cerefox_ingest`, `cerefox_insert`,
`cerefox_edit`, `cerefox_delete_document`, `cerefox_get_document`,
