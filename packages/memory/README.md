...
| `cerefox mcp` | Local stdio MCP server. Drop-in for Claude Code, Cursor, Claude Desktop, Codex CLI, Gemini CLI. Exposes the same 12 MCP tools as the remote `cerefox-mcp` Edge Function. |
...
the 12 MCP tools (`cerefox_search`, `cerefox_ingest`, `cerefox_insert`,
