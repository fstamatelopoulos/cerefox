...
- **Review workflow** (v1.13.0, #241): `review_status` is governed by the store-level `review_workflow_enabled` flag in `cerefox_config` — **false** on a fresh install (seeded by `schema.sql`), **true** on an upgraded store (seeded by migration 0031). The write-side decision lives ONCE in `cerefox_ingest_document`, from `author_type` alone (`p_review_status` is accepted and ignored) — never reintroduce the `author_type === "agent" ? "pending_review" : "approved"` ternary in a client. **The flag is a presentation switch and is NOT consulted on the write path** (v1.13.1, schema 0.16.1): an off store records the same statuses an on store would, so toggling never changes what a stored value means. The read side uses one shared reader, `reviewWorkflowEnabled()` in `_shared/mcp-tools/feature-flags.ts` (15 s cache, fail-closed; the web config route busts it on `PUT`): when off, the field is **absent** from every surface (API rows, MCP output, CLI columns/`--json`, the `cerefox-metadata-search` EF, web UI), `?review_status=` on search is a 400 and `POST …/review-status` is a 404. Stored rows are never touched by a toggle. Spec: `docs/specs/review-workflow-toggle.md`. Any new surface that would show `review_status` must ask the reader first.
- **Search response contract** (v1.14.3, #254-#267): four rules, and every one of them was learned the hard way. (1) **Never report an empty result when rows matched** — an agent reads "No results found." as "this knowledge does not exist", stops looking, and recreates the document; when nothing fits the budget, return the headers of what matched. (2) **Never hold results back silently** — the "N of M shown" notice is the one piece of framing that is never dropped. (3) **The reply is budgeted in RENDERED bytes**, the unit the caller receives, not in `JSON.stringify` bytes; the Edge Function ships JSON and budgets in JSON, and the two must not be confused. It may exceed `max_bytes` only by framing that cannot be dropped without misleading the caller, and that framing is never traded for content. (4) **A result that does not fit is skipped, not the end of the list**, so the returned set is not a rank prefix. Anything added to a search response must be measured against `_shared/__tests__/search-budget-invariant.test.ts` (517 cases over budgets × sizes × counts × row shapes) — seven consecutive review rounds found bugs that the example-based tests passed straight through. Contract for users: `docs/guides/response-limits.md`.
- **Runtime config catalog**: every `cerefox_config` key is declared ONCE in `_shared/config-catalog/index.ts` (`CONFIG_CATALOG`: kind, default, group, `highImpact`, description). The web Settings page, `cerefox config list` (#239) and value validation all derive from it, and `_shared/__tests__/config-catalog-allowlist.test.ts` pins it to the `v_allowed` list in `cerefox_set_config`. Adding a key = catalog entry + `v_allowed` (in `rpcs.sql` AND a migration re-shipping `cerefox_set_config`) + a seed row in `schema.sql`; nothing else to hand-list.
- **Caller identity (v1.13.1)**: every MCP tool takes the caller's name as **`author`** (reads and writes); `requestor` is accepted as a silent alias via `callerIdentity()` in `_shared/mcp-tools/identity.ts`; on `cerefox_get_audit_log` the entries filter is `by_author` so `author` means the same thing there. Schemas list one name; never add a second identity key to a tool schema. Since v1.13.2 (#244) the CLI (`--author` on every command via `packages/memory/src/cli/util/identity-flags.ts`, `--requestor` hidden alias, `--by-author` on `audit list`) and the primitive EFs (`callerIdentity(body)`, `by_author` on `cerefox-get-audit-log`) follow the same rule; the GPT Actions OpenAPI block is at 4.0.0 for it.
- **Requestor enforcement**: optional `require_requestor_identity` config (default false) makes the identity (`author`, or the `requestor` alias) mandatory, and optional `requestor_identity_format` validates them against a regex. Both controlled via `cerefox_config`. **Scope, stated precisely because the name promises more than it delivers: these are enforced by the Edge Functions only** — each of the 9 carries its own copy of the check, including `cerefox-mcp` (the REMOTE MCP transport). The **local stdio MCP server does not enforce them, and neither does `/api/v1`.** So enabling it does not make identity required everywhere. Making it global means one shared helper every transport calls, not a tenth copy of the block.
...
**Ingestion**: The ingestion pipeline has two steps: (1) chunking + embedding (requires external HTTP calls, runs in TypeScript), and (2) database writes (insert document, insert chunks, snapshot version, decide + set review_status from `author_type` and the `review_workflow_enabled` flag, create audit entry). Step 2 is handled entirely by the `cerefox_ingest_document` RPC -- a single atomic transaction. Both the TS `IngestionPipeline` and the `cerefox-ingest` Edge Function call this RPC after completing step 1. This ensures all write logic, review_status transitions, and audit entry creation happen in one place.
...
7. **Chunks-anchored versioning** — `version_id IS NULL` = current version; `version_id = <uuid>` = archived; partial indexes automatically exclude archived chunks from search; no separate content table; archived chunks carry **no search artifacts** (embeddings/fts nulled at archive time, 0.13.0 #216) — their `content` is the safety copy, and restore is manual re-ingest
...
9. **Optimistic concurrency on content updates** (v0.11 / schema 0.5.0) — updates via `cerefox_ingest_document` require `p_expected_content_hash` (compare-and-swap against the document's existing `content_hash`, atomic via `SELECT … FOR UPDATE` in the RPC) or an explicit `p_last_write_wins` (audit-logged). Stale → `CEREFOX_CONFLICT` (`PT409` / HTTP 409); absent **or blank** → `CEREFOX_TOKEN_REQUIRED` (22023 / HTTP 400). The conflict code was `40001` until v1.1.0-beta.6; that is `serialization_failure`, which infrastructure treats as retryable, and a deterministic conflict under it caused a 47-million-call retry storm. Never raise a permanent error under a retryable SQLSTATE. All document-shaped read RPCs return `content_hash` so writers hold the token. Filesystem-sync flows (`ingest-dir`, `guides ingest`) pass `last_write_wins` internally. Design: `docs/specs/concurrency-control-design.md`.
...
| `docs/plans/history.md` | Closed iterations (master history log) | An iteration closes |
...
**`docs/plan.md` is the primary cross-session hand-off artifact** — its main consumer is the *next* AI session continuing the work. It is deliberately short: `## Current Focus` is at the **top**, followed by the active iteration, whose detailed plan lives in its own file under `docs/plans/`. Closed iterations move to `docs/plans/history.md` (the master history log, which indexes the per-iteration plans). Read `Current Focus` first to learn where the project is and what's next before touching code, and **keep it current as part of finishing any work** (update the iteration's plan + `Current Focus` in the same session).
...
- **Agent guides**: `AGENT_GUIDE.md` (comprehensive reference for AI agents using Cerefox tools), `AGENT_QUICK_REFERENCE.md` (minimal quick reference card -- 15 core tools, key rules, workflows)
