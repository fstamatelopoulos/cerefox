...
- **Review workflow** (v1.13.0, #241): `review_status` is governed by the store-level `review_workflow_enabled` flag in `cerefox_config` — **false** on a fresh install (seeded by `schema.sql`), **true** on an upgraded store (seeded by migration 0031). The write-side decision lives ONCE in `cerefox_ingest_document` (`cerefox_config_bool('review_workflow_enabled', FALSE)`; `p_review_status` is accepted and ignored) — never reintroduce the `author_type === "agent" ? "pending_review" : "approved"` ternary in a client. The read side uses one shared reader, `reviewWorkflowEnabled()` in `_shared/mcp-tools/feature-flags.ts` (15 s cache, fail-closed; the web config route busts it on `PUT`): when off, the field is **absent** from every surface (API rows, MCP output, CLI columns/`--json`, the `cerefox-metadata-search` EF, web UI), `?review_status=` on search is a 400 and `POST …/review-status` is a 404. Stored rows are never touched by a toggle. Spec: `docs/specs/review-workflow-toggle.md`. Any new surface that would show `review_status` must ask the reader first.
