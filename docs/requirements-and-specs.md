...
| FR-11.5 | Archived chunks MUST NOT carry search artifacts: embeddings and FTS are nulled at archive time (v1.8.0, #216) — the version's `content` is the safety copy; search is current-chunks-only; restore is manual re-ingest (re-embeds) | P1 |
