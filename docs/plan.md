...
### Iteration 33 — Partial Document Edits (2026-08-08 → , target v1.3.0-beta)
...
**Build (branch `feat/partial-edits`)**:
1. ✅ Freeze-pass on the spec (uniform nesting rule for replace/delete)
2. ✅ Technical design doc
3. ✅ `_shared/partial-edits/` pure module (outline parser, anchor resolver,
   operation applier) + 40 unit tests
4. ✅ DB: schema **0.11.0**, migration 0019 (audit CHECK widened),
   `p_operations` per-op audit entries, `content_hash` on create (#189),
   `size_warning` + `document_size_warning_chars`. Staging probe 11/11; two
   defects found there and fixed (audit entries within a batch shared
   `NOW()` so their order was unrecoverable → `clock_timestamp()`; the config
   key was catalogued but missing from `cerefox_set_config`'s in-RPC allow-list)
...
7. ⏳ Live e2e vs **staging only**; verify against the spec §3/§9
...
feedback rounds, technical design committed, build in progress toward a
staging-validated beta. See the iteration 33 entry above for the step list and
current state; the frozen spec is the success criterion. Staging is unpaused for
the build and carries schema 0.11.0; **prod untouched until joint validation
passes**. Awaiting: feedback from Debasis's agents on the spec (slots into §8/§9
without reopening the freeze unless a session finding demands it). #189 ships
...
**Phase 2 — the headline feature. ✅ CORE SHIPPED (2026-08-05).** Delivered:
`cerefox_document_relations` (typed directed edges, free-text `rel_type`,
symmetric-type dictionary in SQL), `lifecycle_status` on documents, four RPCs
(`set_relation`, `delete_relation`, `get_relations`, `get_neighbors` with a
cycle-safe recursive walk), four MCP tools (10 → 14), the `cerefox relation`
CLI group, migration 0014, schema 0.10.0. Validated by a 16-scenario suite on a
throwaway Postgres (symmetry, upsert idempotence, lifecycle side effects,
self-edge + missing-document rejection, depth-2 chains, cycle termination,
cascade on hard delete, soft-delete hiding, audit entries) plus 13 tool-layer
unit tests. **Deliberately deferred to a follow-on slice**: relation-aware
search (traversal in `cerefox_search_docs` + relation summaries in result rows)
and the web UI relation panel — re-opening search ranking so soon after the
1.0.3–1.0.6 stabilisation is not a risk worth taking unattended.
