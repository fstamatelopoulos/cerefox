...
- **✅ Tests added for the new commands (v0.9.1)**: cli-smoke `--help` smoke for `document edit/restore/version`, `project create/edit`, `config list`, `search --only-metadata`, `completion install`; plus a deterministic sandbox-HOME behavior test for `completion install` (writes script + idempotent rc block). Full suite 120 pass. (Live write-command round-trips — project create/edit/delete, document edit patch, document restore — were validated by hand this session; adding them to the live `write-commands.test.ts` probe-and-skip suite is a follow-up.)
