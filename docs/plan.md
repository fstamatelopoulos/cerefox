...
| **`document edit`** | ⬜ open — needs design | Confirmed: content edits already work via `document ingest --document-id --update` (re-supplying content; content-unchanged → metadata-only path). The *new* value of `edit` is **non-destructive title/metadata patch** (the thing `ingest --update` does NOT give — see the metadata footgun above) + title-boost re-embed on title change. Lean: `document edit <id> [--title] [--set-meta JSON]` that fetches existing metadata and merges; content stays on `ingest --update`. |
...
| **`search` deeper parity (EASY — RPC already returns it)** | ⬜ quick win | `cerefox_search_docs` already RETURNS `best_chunk_heading_path` (best-match breadcrumb) + `doc_updated_at`; the CLI's `DocResult` just doesn't read them. So web "best match" + "last updated" are **no extra processing** — add both to the CLI metadata line for parity. No RPC change. (Best-chunk full *text* would need a new RPC field — defer.) |
| **`completion` UX (cfcf pattern confirmed)** | ⬜ decide | cfcf has `cfcf completion install` that writes `~/.cfcf-completion.{bash,zsh}` + idempotently appends a **sentinel-marked block** to the shell rc (`# >>> cfcf shell completion >>>` … `<<<`), wired via `install.sh` + npm `postinstall` + `self-update`; user runs `exec zsh`/`source` after. Mirror for cerefox: add `cerefox completion install`, wire into `install.sh` (with a confirmation, since it edits the rc). |
