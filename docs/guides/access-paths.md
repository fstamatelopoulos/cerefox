...
| 2. Soft-destructive | `delete_document` (soft delete to trash), `set_review_status` | yes — restorable via web UI | CLI (`cerefox document delete`), web UI, and — since v1.7.0 (#208) — MCP (`cerefox_delete_document`, which requires the caller's read-hash). **Not** the primitive GPT-Actions Edge Functions (deliberately deferred). |
