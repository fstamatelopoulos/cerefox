...
| **npm auth bootstrap** | Sketched as "OIDC trusted publishing" without the chicken-and-egg note | Reuses cfcf's proven two-phase pattern: one-shot `NPM_TOKEN` for the first publish (bypass-2FA, revoked same day) → register OIDC Trusted Publisher → all subsequent publishes use OIDC with `--provenance`. Captured in detail in cfcf's 2026-05-01 decision-log entry (already in Cerefox; search "npm publish auth OIDC trusted publishing"). | `docs/plan.md` § Iteration 22 refinement #9, "Manual prerequisites" subsection, 22F |
